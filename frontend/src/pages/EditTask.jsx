...
function EditTask() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const formattedDueDate = taskData.dueDate ? new Date(taskData.dueDate).toISOString().split('T')[0] : '';
  ...
  const removeExistingFile = async (fileId) => {
    try {
      setIsLoading(true);
      // In a real app, you might want to make an API call to delete the file from the server
      // For now, we'll just remove it from the UI
      setExistingDocuments(prev => prev.filter(file => file._id !== fileId));
    } catch (error) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // Send existing documents as JSON string or individual IDs
      // Option 1: Send as JSON string
      if (existingDocuments.length > 0) {
        formDataToSend.append('existingDocuments', JSON.stringify(existingDocuments));
      }
      
      // Option 2: Send individual document IDs (alternative approach)
      // existingDocuments.forEach(doc => {
      //   formDataToSend.append('existingDocumentIds[]', doc._id || doc.id);
      // });
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg font-medium text-gray-700">Loading task data...</p>
        </div>
      </div>
    );
  }
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-2">
                    {existingDocuments.map((file) => (
                      <div key={file._id} className="flex items-center justify-between p-3 bg-gray-100 rounded-lg">
                        <div className="flex items-center space-x-2">
                          ...
                          <a 
                            href={`http://localhost:8000/${file.path || file}`} 
                            target="_blank" 
